...
router.get("/usuarios/:name", async (req, res) => {
  ...
  const usuario = await pool.query(
    "select username,id, pregunta_seguridad from usuarios where username = ?",
    [name]
  );
  res.json(usuario);
});
// searc usuario
router.get("/usuarios/respuesta/:id", async (req, res) => {
  const { id } = req.params;
  const usuario = await pool.query(
    "select id, respuesta from usuarios where id = ?",
    [id]
  );
  res.json(usuario);
});
